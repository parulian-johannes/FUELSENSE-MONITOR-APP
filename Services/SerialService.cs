...
namespace EngineMonitoring.Services
{
    ...
    private readonly string debugLogPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory ?? ".", "serial_debug.log");

    // Last known snapshot of sensor values (updated as partial lines arrive)
    private SensorData lastKnown = new SensorData();
    private DateTime lastEmitTime = DateTime.MinValue;
    private readonly object lastKnownLock = new();
        ...
        private void StartDataReading()
        {
            ...
            Task.Run(async () =>
            {
                ...
                Console.WriteLine($">>> RAW DATA FROM LOGGER: '{rawData}'");
                                ...
                                // POST-PROCESS: Jika temperature masih missing (NaN) dan kita punya possible temp, gunakan itu
                                if (double.IsNaN(sensorData.Temperature) && possibleTemp.HasValue)
                                {
                                ...
                                // Aggregate into lastKnown snapshot and emit at most once per second
                                lock (lastKnownLock)
                                {
                                    // Update only fields that were provided in this parsed line
                                    // Update lastKnown snapshot with any values present in this packet.
                                    // Note: treat RPM >= 0 as a valid reading (0 = engine off). Previously we
                                    // ignored RPM==0 which caused the UI to keep showing -1.
                                    if (!double.IsNaN(sensorData.Torque)) lastKnown.Torque = sensorData.Torque;
                                    if (!double.IsNaN(sensorData.Fuel)) lastKnown.Fuel = sensorData.Fuel;
                                    if (!double.IsNaN(sensorData.Temperature)) lastKnown.Temperature = sensorData.Temperature;
                                    if (!double.IsNaN(sensorData.MAF)) lastKnown.MAF = sensorData.MAF;
                                    if (sensorData.RPM >= 0) lastKnown.RPM = sensorData.RPM;

                                    var now = DateTime.Now;
                                    // Emit once per second (real-time cadence)
                                    if ((now - lastEmitTime).TotalSeconds >= 1)
                                    {
                                        var emit = new SensorData
                                        {
                                            Time = now,
                                            Torque = lastKnown.Torque,
                                            Fuel = lastKnown.Fuel,
                                            Temperature = lastKnown.Temperature,
                                            MAF = lastKnown.MAF,
                                            RPM = lastKnown.RPM
                                        };
                                            DebugLog($"EMIT: Time={emit.Time:O} RPM={emit.RPM} Torque={emit.Torque} Fuel={emit.Fuel} Temp={emit.Temperature} MAF={emit.MAF}");
                                        lastEmitTime = now;
                                        // Fire event outside lock to avoid reentrancy issues
                                        Task.Run(() => DataReceived?.Invoke(this, emit));
                                    }
                                }
                            }
                        }
        ...
        private SensorData ParseSensorData(string rawData)
        {
            ...
            // Parse numbers: prioritize DOT decimal (from microcontroller), fallback to comma
            var ciDot = CultureInfo.InvariantCulture;
            var ciComma = new CultureInfo("id-ID");
            ciComma.NumberFormat.NumberGroupSeparator = ""; // DISABLE thousands separator
            ciComma.NumberFormat.NumberDecimalSeparator = ",";
            ...
            try
            {
                ...
                {
                    // ambil angka, minus, titik, dan koma
                    var chars = s.Trim();
                    var buf = new System.Text.StringBuilder(chars.Length);
                    foreach (var ch in chars)
                        if ((ch >= '0' && ch <= '9') || ch == '.' || ch == ',' || ch == '-' ) buf.Append(ch);
                    return buf.ToString();
                }

                bool TryParseDouble(string s, out double value)
                {
                    // PRIORITAS: Dot decimal dari microcontroller (28.00 → 28.00, bukan 2800!)
                    // NumberStyles.Float allows negative, decimal point, but NO thousands separator
                    if (double.TryParse(s, NumberStyles.Float | NumberStyles.AllowLeadingWhite | NumberStyles.AllowTrailingWhite, ciDot, out value))
                        return true;
                    // Fallback: comma decimal (28,00 → 28.00)
                    return double.TryParse(s, NumberStyles.Float | NumberStyles.AllowLeadingWhite | NumberStyles.AllowTrailingWhite, ciComma, out value);
                }
                ...
                if (rawData.Contains(","))
                {
                    ...
                    // DETEKSI FORMAT LOGGER LANGSUNG - PRIORITAS TERTINGGI
                    // Format logger: kolom 1=Torque, 2=Fuel, 3=MAF, 4=RPM, 5=Temperature
                    // Kolom 0 bisa kosong atau nilai kecil (abaikan)
                    // RPM ada di kolom 4, APAPUN nilainya (termasuk 0 untuk engine off)
                    
                    if (vals.Length >= 5)
                    {
                        Console.WriteLine($"[CSV] Detected {vals.Length} columns - applying DIRECT mapping");
                        Console.WriteLine($"[CSV] Raw values: Col0='{vals[0]}' Col1='{vals[1]}' Col2='{vals[2]}' Col3='{vals[3]}' Col4='{vals[4]}'");
                        
                        // MAPPING LANGSUNG - TIDAK PEDULI FORMAT
                        // Kolom 1 = Torque
                        if (vals.Length > 1 && !double.IsNaN(nums[1])) 
                        {
                            sensorData.Torque = nums[1];
                            Console.WriteLine($"[CSV] Torque from col[1]: {sensorData.Torque}");
                        }
                        
                        // Kolom 2 = Fuel
                        if (vals.Length > 2 && !double.IsNaN(nums[2])) 
                        {
                            sensorData.Fuel = nums[2];
                            Console.WriteLine($"[CSV] Fuel from col[2]: {sensorData.Fuel}");
                        }
                        
                        // Kolom 3 = MAF
                        if (vals.Length > 3 && !double.IsNaN(nums[3])) 
                        {
                            sensorData.MAF = nums[3];
                            Console.WriteLine($"[CSV] MAF from col[3]: {sensorData.MAF}");
                        }
                        
                        // Kolom 4 = RPM - INI YANG PENTING!
                        // SELALU ambil dari kolom 4, tidak peduli nilainya 0 atau berapa
                        if (vals.Length > 4 && !double.IsNaN(nums[4])) 
                        {
                            sensorData.RPM = (int)Math.Round(nums[4]);
                            Console.WriteLine($"[CSV] RPM from col[4]: {sensorData.RPM} *** FORCED FROM COLUMN 4 ***");
                        }
                        else
                        {
                            Console.WriteLine($"[CSV] WARNING: Column 4 is NaN or missing! Raw value: '{vals[4]}'");
                        }
                        
                        // Kolom 5 = Temperature (jika ada)
                        if (vals.Length > 5 && !double.IsNaN(nums[5])) 
                        {
                            sensorData.Temperature = nums[5];
                            Console.WriteLine($"[CSV] Temperature from col[5]: {sensorData.Temperature}");
                        }
                        
                        Console.WriteLine($"[CSV DIRECT RESULT] RPM={sensorData.RPM} Torque={sensorData.Torque} Fuel={sensorData.Fuel} MAF={sensorData.MAF} Temp={sensorData.Temperature}");
                        return sensorData;
                    }

                    // Use header mapping if available and width matches
                    if (HasHeaderMappingFor(vals.Length))
                    {
                        AssignUsingHeader(nums, sensorData);
                        Console.WriteLine($"[CSV HEADER MAP] Assigned using header mapping for {vals.Length} columns. Result RPM:{sensorData.RPM} Torque:{sensorData.Torque}");
                    }
                    else if (HasValidCsvMapping(nums))
                    {
                        // Use global calibrated mapping if available
                        AssignCsvValues(nums, sensorData);
                        Console.WriteLine($"[CSV GLOBAL MAP] Assigned using global mapping. Result RPM:{sensorData.RPM} Torque:{sensorData.Torque}");
                    }
                    else
                    {
                        // Fallback: compute temporary heuristic mapping for this row
                        var tempMap = ComputeHeuristicMapping(nums);
                        AssignCsvValues(nums, sensorData, tempMap);
                        Console.WriteLine($"[CSV HEURISTIC MAP] Assigned using heuristic mapping. Result RPM:{sensorData.RPM} Torque:{sensorData.Torque}");

                        // Add to calibration buffer to improve future global mapping
                        AddCsvRowForCalibration(nums);
                        TryCalibrateCsvMapping();
                    }

                    // Final CSV-level fallback: jika RPM belum terdeteksi, coba deteksi index RPM dari baris ini
                    if (sensorData.RPM <= 0)
                    {
                        int rpmIdx = DetectRpmIndexSingle(nums);
                        if (rpmIdx >= 0 && rpmIdx < nums.Length && !double.IsNaN(nums[rpmIdx]))
                        {
                            sensorData.RPM = (int)Math.Round(nums[rpmIdx]);
                            Console.WriteLine($"[CSV FALLBACK] RPM detected at index {rpmIdx}, value: {sensorData.RPM}");
                        }
                    }
                ...
                if (!double.IsNaN(sensorData.Temperature) && (sensorData.Temperature < 0 || sensorData.Temperature > 500))
                {
                    Console.WriteLine($"[TEMP WARNING] Temperature value {sensorData.Temperature} seems unreasonable, keeping as-is");
                }

                // ADDITIONAL TEMPERATURE DETECTION: Try to find temperature value if it's still missing
                if (double.IsNaN(sensorData.Temperature) && !string.IsNullOrEmpty(rawData))
                {
        ...
        private CsvMap ComputeHeuristicMapping(double[] nums)
        {
            ...
            // RPM: nilai besar (biasanya >=300..10000). Mode agresif: ambil kolom terbesar jika memenuhi
            // salah satu kriteria berikut (urutan prioritas):
            // - Jika maxVal >= 600 -> accept (biasanya RPM mesin sudah idle/putaran rendah)
            // - Jika maxVal >= 400 dan ratio >= 1.3 -> accept (lebih besar dari lainnya)
            // - Jika maxVal >= 300 dan ratio >= 1.5 -> accept (jauh lebih besar)
            int rpmIdx = -1;
            ...
            for (int i = 0; i < nums.Length; i++)
                if (i != rpmIdx && !double.IsNaN(nums[i]) && nums[i] > second) second = nums[i];

            // Kriteria penerimaan yang lebih permisif
            bool accept = false;
            if (maxVal >= 600)
            {
                accept = true; // cukup besar, kemungkinan besar RPM
            }
            else if (maxVal >= 400 && second > 0 && (maxVal / second) >= 1.3)
            {
                accept = true; // lebih besar dari kolom lain
            }
            else if (maxVal >= 300 && second > 0 && (maxVal / second) >= 1.5)
            {
                accept = true; // jauh lebih besar
            }

            if (!accept)
            {
                // tidak memenuhi kriteria, batalkan deteksi RPM
                rpmIdx = -1;
            }
        ...
        // Simple debug logger to file (thread-safe, best-effort)
        ...
        {
            // Aggressive single-row RPM detection to capture RPM from logger.
            // Rules (tiered, most permissive):
            // 1) If max >= 600 -> accept (typical idle+ RPM)
            // 2) If max >= 400 and ratio >= 1.3 -> accept (moderately larger)
            // 3) If max >= 300 and ratio >= 1.5 -> accept (clearly larger)
            // 4) If max >= 250 and ratio >= 2.0 -> accept (very distinct)
            ...
            double second = double.MinValue;
            for (int i = 0; i < nums.Length; i++)
                if (i != bestIdx && !double.IsNaN(nums[i]) && nums[i] > second) second = nums[i];

            // Tiered acceptance
            if (max >= 600) return bestIdx;
            if (max >= 400 && second > 0 && (max / second) >= 1.3) return bestIdx;
            if (max >= 300 && second > 0 && (max / second) >= 1.5) return bestIdx;
            if (max >= 250 && second > 0 && (max / second) >= 2.0) return bestIdx;
        ...
        // [BARU] API umum untuk mengirim perintah ke device (Arduino)
        public void SendCommand(string command)
        {
            try
            {
                if (IsConnected && serialPort != null && serialPort.IsOpen)
                {
                    ...
                    Console.WriteLine($"[SERIAL TX] {command}");
                }
                else
                {
                    Console.WriteLine("[SERIAL TX] Port belum terbuka");
                }
            }
            catch (Exception ex)
            {
